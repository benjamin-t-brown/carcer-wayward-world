...
async function getSdl2wAssetFileContents() {
    ...
    const assetFiles = files.filter((f) => f.startsWith('assets.') && f.endsWith('.txt'));
    const assetFilesContent = {};
...
function contentTypeForAssetFile(filePath) {
...
async function serveSrcAssetFile(req, res, notFoundLabel) {

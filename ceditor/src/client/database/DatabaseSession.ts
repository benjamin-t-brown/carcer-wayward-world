import { ASSET_TYPES, type AssetId } from '../../shared/assetRegistry';
import type {
  DatabaseEnvelope,
  DatabaseSnapshot,
  DatabaseTransport,
...
export class DatabaseSession {
  ...
  async saveAll(): Promise<SaveDatabaseResponse> {
    const operation = this.saveTail
      ? this.saveTail.then(() => this.saveAllNow())
      : this.saveAllNow();
  ...
  private async saveAllNow(): Promise<SaveDatabaseResponse> {

...
export function VariableEditorModal({
  ...
  const [vars, setVars] = useState<Variable[]>(
    gameEvent?.vars
      .filter((variable) => variable.importFrom === '')
  ...
  const handleConfirm = () => {
    const newVars = vars.map((variable) => structuredClone(variable));
    ...
    for (const variable of importVars) {
      ...
      newVars.push(structuredClone(variable));
    }
    if (errors.length > 0) {
      ...
      if (gameEvent) {
        const updatedGameEvent = structuredClone(gameEvent);
        updatedGameEvent.vars = newVars;
        setErrorText('');
